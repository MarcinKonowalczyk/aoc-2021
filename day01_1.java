...
public class day01_1 {
  ...
  public static void main(String args[]) {
    ...
    try {
      ...
      depths = load("data/test/day01_input.txt");
    } catch (IOException e) {
      e.printStackTrace();
      System.exit(1);
    }

    // List<Integer> diffs = windowed(depths, 2).stream().map(i ->
    // i.get(1)-i.get(0)).collect(Collectors.toList());
    // System.out.println(diffs);

    long increases =
        windowed(depths, 2).stream().map(i -> i.get(1) - i.get(0)).filter(i -> i > 0).count();
