...
public class day01_2 {
    ...
    public static void main(String args[]) {
        // List<Integer> depths = load("data_full/day01_input.txt");
        List<Integer> depths = load("data/day01_input.txt");
        long increases =  windowed(windowed(depths, 3).stream().map(i -> i.stream().reduce(0, (x,y)->x+y)).collect(Collectors.toList()), 2).stream().map(i -> i.get(1)-i.get(0)).filter(i->i>0).count();
